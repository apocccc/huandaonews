    ...
    <div className="mx-auto max-w-3xl px-4 py-6">
      ...
      <article className="mt-4">
        ...
        <header>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 border-y border-line py-3 text-sm text-gray">
            <span>
              {t("labels.byAuthor")}{" "}
              <Link
          ...
          {article.lead ? (
            <p className="mt-5 text-[17px] font-medium leading-relaxed text-ink">
              {article.lead}
            </p>
          ) : null}
        ...
        {article.tags.length > 0 ? (
          <footer className="mt-10 border-t border-line pt-5">
            <p className="text-xs font-bold text-gray">{t("labels.tags")}</p>
            <ul className="mt-2 flex flex-wrap gap-2">
              {article.tags.map(({ tag }) => (
                <li key={tag.id}>
                  <Link
                    href={`/tag/${tag.slug}`}
                    className="inline-block rounded-full border border-line px-3 py-1 text-sm text-gray hover:border-primary hover:text-primary"
                  >
                    #{tag.nameZh}
                  </Link>
                </li>
      ...
      {related.length > 0 ? (
        <section className="mt-14" aria-label={t("labels.relatedArticles")}>
          <SectionHeading>{t("labels.relatedArticles")}</SectionHeading>
          <div className="mt-5 flex flex-col gap-6">
            {related.map((a) => (
              <ArticleCard key={a.id} article={a} locale={locale} showCategory={false} />
            ))}
          </div>
        </section>
      ) : null}
